...
class GeneralSettings extends Component {
  ...
  handleToChange = e => {
    ...
    if (value.length > 0) {
      const regex = new RegExp(`^${value}`, 'i')
      toEmailSuggestions = searchOptions.sort().filter(v => regex.test(v.email))
    }
  ...
  handleCcChange = e => {
    ...
    if (value.length > 0) {
      const regex = new RegExp(`^${value}`, 'i')
      ccEmailSuggestions = searchOptions.sort().filter(v => regex.test(v.email))
    }
